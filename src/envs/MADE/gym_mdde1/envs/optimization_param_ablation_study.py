###########################################################################################################
# GLo-MAPPO: Multi-Flying LoRa UAVs Env -- Optimization-Parameter Ablation
...
def generate_uav_positions(num_uavs, altitude):
    ...
    # Generic fallback for >5 UAVs: continue the same start row (x = 50,150,250,...
    # at y=50, z=altitude), consistent with the predefined 2-5 layouts.
...
def compute_pathloss(n_pos, g_pos, f):
    ...
    d_horiz = distance.euclidean(n_pos[:2], g_pos[:2])  

    # Elevation angle from the horizontal distance.
    # atan2 handles d_horiz = 0 (UAV directly overhead) -> 90 degrees, correctly.
...
def compute_snr(tp_dbm, Ed_pos, Gw_pos, f, gain_linear=None, sigma_noise_w=None):
    ...
    if sigma_noise_w is None:
        sigma_noise_w = 10 ** ((-117 - 30) / 10)   # fallback if not supplied
...
def propulsion_power(V_array): # Expects V as a numpy array of speeds
...
class MultiFlyingLoRaEnv(gym.Env):
    def __init__(self, num_uavs, num_eds, area_size, uav_altitude, 
        ...
        self._apply_opt_ablation(kwargs.get('opt_mode', None))
        ...
        self.max_ed_per_uav = max(1, num_eds // num_uavs)  # simple even split; can be tuned or made dynamic
        ...
        # --- Global state for the centralized critic (paper State Space, for s[t]) ---
    ...
    # ---- OPTIMIZATION-PARAMETER ABLATION (SF / TP / position) ----
    # Shows the impact of GLo-MAPPO's joint optimization by FIXING one (or all) of the
    # optimized quantities while the rest stay learned. Paper values: SF=9, TP=8 dBm.
    _OPT_FIXED_SF = 12       # must be in sf_options; SF=12 = standard LoRa default (no ADR, max-SF)
    _OPT_FIXED_TP_DBM = 8    # must be in tp_options
    _VALID_OPT_MODES = ('full', 'fix_sf', 'fix_tp', 'fix_position', 'fix_all')
    _opt_banner_shown = False

    def _apply_opt_ablation(self, opt_mode):
        self.opt_mode = (str(opt_mode).lower() if opt_mode is not None else 'full')
        if self.opt_mode not in self._VALID_OPT_MODES:
            raise ValueError(f"unknown opt_mode='{opt_mode}'. Valid: " + ", ".join(self._VALID_OPT_MODES))
        self._opt_fix_sf = self.opt_mode in ('fix_sf', 'fix_all')        # force chosen_sf = 9
        self._opt_fix_tp = self.opt_mode in ('fix_tp', 'fix_all')        # force chosen_tp = 8 dBm
        self._opt_fix_position = self.opt_mode in ('fix_position', 'fix_all')  # UAVs static (speed 0)
        if not MultiFlyingLoRaEnv._opt_banner_shown:
            tag = {'full': 'FULL optimization (SF+TP+position) -- baseline',
                   'fix_sf': "w/o SF optimization (SF fixed = 9)",
                   'fix_tp': "w/o TP optimization (TP fixed = 8 dBm)",
                   'fix_position': "w/o POSITION optimization (UAVs static)",
                   'fix_all': "w/o ANY optimization (SF=9, TP=8 dBm, static)"}[self.opt_mode]
            print("=" * 64 + f"\n  [OPT ABLATION] {tag}\n" + "=" * 64, flush=True)
            MultiFlyingLoRaEnv._opt_banner_shown = True
    ...
    def reset(self):
        ...
        self._init_dist_to_cs = self.initial_distances_to_cs.copy()   
    ...
    def step(self, actions):
        ...
        for uav_idx, action in enumerate(actions):
           ...
           #############################################################################################################################
            ...
            if self._opt_fix_position:
                s = 0.0   # ABLATION: w/o position optimization -> UAVs stay static
           ...
           #############################################################################################################################
            ...
            self.uav_propulsion_power_this_step[uav_idx] = prop_power_val #* self.config['delta_t'] 
        ...
        # Per-UAV accumulators for instantaneous energy efficiency, Eq. (20)
        ...
        if active_uav_indices and self.num_eds > 0:
            ...
            if self._opt_fix_sf or self._opt_fix_tp:
                for _ci in ed_candidates.values():
                    if self._opt_fix_sf: _ci['sf'] = self._OPT_FIXED_SF
                    if self._opt_fix_tp: _ci['tp'] = self._OPT_FIXED_TP_DBM
