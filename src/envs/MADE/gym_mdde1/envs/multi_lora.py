###########################################################################################################
# GLo-MAPPO: Multi-Flying LoRa UAVs Custom Gym Environment 
...
def generate_uav_positions(num_uavs, altitude):
    ...
    return np.array([[50 + 100 * i, 50, altitude] for i in range(num_uavs)])
...
def compute_pathloss(n_pos, g_pos, f):
    ...
    d = distance.euclidean(n_pos, g_pos)               
...
def compute_sinr_components(tp_dbm, ed_idx, ed_positions, sf, uav_pos,
    ...
    n_co = 0       # #in-range co-SF interferers  
...
def lora_time_on_air(sf, payload_bytes=20, bw=125e3, coding_rate=1,
    ...
    # Low-Data-Rate Optimization is mandatory when t_sym > 16 ms (SF11/SF12 @ 125 kHz)
...
env_config = {
    ...
    'cosf_capture_threshold_db': 6.0,   # q_coSF, fixed for ALL SFs
    ...
    'duty_cycle': 0.01,              # LoRa ED duty cycle (EU868 1%)
    ...
    'rician_A2': 4.39,               # K-factor elevation exponent
    ...
    # ---------- reward weights ----------
    'reward_weights': {
        'omega1_association_indicator': 10.0,        # ω1  association indicator c_u
        'omega2_coverage_bonus': 400.0,              # ω2  (B) coverage bonus per NEW system-unique ED 
        'omega3_system_ee_step':  4e-7,              # ω3  system EE 
        'omega4_spread_incentive': 5.0,              # ω4  (D) inter-UAV spread reward per step 
        'omega5_collision_penalty': 200.0,           # ω5  collision indicator D_safe
        'omega6_distance_to_cs_penalty': 2.0,        # ω6  CS distance
        'omega7_cs_arrival_failure_penalty': 400.0,  # ω7  terminal CS-arrival failure panalty (per-agent, if not at CS at episode end)
        'spread_cap': 250.0,                         # δ_cap  separation [m] at which the spread reward saturates
    },
...
class MultiFlyingLoRaEnv(gym.Env):
    def __init__(self, num_uavs, num_eds, area_size, uav_altitude, 
        ...
        self.max_ed_per_uav = max(1, num_eds // num_uavs)  # quota of EDs per UAV 
        ...
        # ED-slot features: [x, y, dist_to_uav, faded_gain].
        ...
        self.n_actions = sum(self.action_nvec)  
        _per_agent_space = MultiDiscrete(self.action_nvec)

        # The action space is a tuple of MultiDiscrete spaces, one for each agent, where each MultiDiscrete has the same action_nvec.
        ...
        # The observation space is a tuple of Box spaces, one for each agent, where each Box has shape.
    ...
    def initialize_tracking_variables(self):
        ...
        # START: Attributes for storing reward components
        ...
        self.episode_sum_weighted_r_coverage = 0.0   # (B) coverage bonus accumulator
        self.episode_sum_weighted_r_spread = 0.0     # (D) spread reward accumulator

        self.episode_sum_raw_a_u_t = 0.0
        self.episode_sum_raw_EE_sys_at_each_step = 0.0
        self.episode_sum_norm_EE_sys_at_each_step = 0.0 
    ...
    def reset(self):
        ...
        # Closed-loop measured-gain injection (set_measured_gain); None => open-loop
        # (analytic channel model). Reset per episode so a new episode starts clean.
    ...
    def get_state(self):
            ...
            # G_{u,v} stored as faded gain in dB: 10*log10(G_{u,v} * |h_{u,v}|^2).
            # Uses the same per-step Rician realization (self.fading_matrix) as association,
            # SNR, and SINR, so the critic observes the actual channel the agents experience.
            # Linear gain (~1e-11 to 1e-8) is numerically negligible in a linear layer;
            # dB scale keeps conventional sign and matches the magnitude of other features.
    ...
    def _decode_action(self, action):
        # action layout: [direction, speed, (sf_v, tp_v) for v in 0..M-1]
    ...
    def step(self, actions):
        ...
        # Per-ED-slot SF/TP action indices for this step (raw head outputs; 0 = null).
        # Bound to actual EDs during association via the cached observation slot map.
        M = self.max_ed_per_uav
        ...
        for uav_idx, action in enumerate(actions):
           ...
           #############################################################################################################################
            ...
            direction_unit_to_cs = h_vec / norm_h                   #if norm_h > 1e-6 else np.array([0.0, 0.0]) 
            ...
            # Enforce propulsion-power budget P_max: regulate speed so that
            # P_prop(s) <= P_max. If the chosen speed exceeds the budget, clamp down to
            # the highest admissible speed level.
        ...
        # Per-UAV accumulators for instantaneous energy efficiency
        per_uav_rate_this_step = np.zeros(self.n_agents)
        per_uav_power_this_step = np.zeros(self.n_agents)
        per_uav_served_eds_this_step = np.zeros(self.n_agents, dtype=int)       
        per_uav_new_eds_this_step = np.zeros(self.n_agents)                
        ...
        if active_uav_indices and self.num_eds > 0:
            ...
            for ed_idx in range(self.num_eds):
                best_active_uav_for_ed, max_gain_for_ed = None, -1.0
                best_sf_raw, best_tp_raw = 0, 0
                for i_active in range(len(active_uav_indices)):
                    ...
                    # Per-ED action binding: a UAV can serve an ED only if (a) it observed
                    # that ED (so an action head exists for it) and (b) it did not pick the
                    # null/decline action (index 0) on that ED's SF or TP slot.
                    k = self._obs_ed_slot[u].get(ed_idx)
                    if k is None:
                        continue
                    if sf_raw_all[u][k] == 0 or tp_raw_all[u][k] == 0:
                        continue
                    uav_pos = _uav_positions_active[i_active]
                    ed_pos = self.ed_positions[ed_idx]
                    if distance.euclidean(ed_pos[:2], uav_pos[:2]) > self.comm_range:
                        continue
                    gain = compute_gain(ed_pos, uav_pos, self.config['frequency'],
                                        fading_power=self.fading_matrix[u, ed_idx])
                    if gain > max_gain_for_ed:
                        max_gain_for_ed, best_active_uav_for_ed = gain, i_active
                        best_sf_raw, best_tp_raw = sf_raw_all[u][k], tp_raw_all[u][k]

                if best_active_uav_for_ed is not None:
                    original_uav_idx_assigned = active_to_original_uav_idx[best_active_uav_for_ed]
                    ed_candidates[ed_idx] = {
                        'uav_idx': original_uav_idx_assigned, 'gain': max_gain_for_ed,
                        'sf': self.config['sf_options'][best_sf_raw - 1],
                        'tp': self.config['tp_options'][best_tp_raw - 1],
                    }
                    _current_assignment_counts_active[best_active_uav_for_ed] += 1
            ...
            for ed_idx, C_info in ed_candidates.items():
                ...
                # Closed-loop: FLoRa's MEASURED channel gain drives the reception decision, so
                # Tier-2's independent Oulu + shadowing channel actually affects coverage / PDR /
                # EE (not just the observation). Falls back to the analytic gain when no gain has
                # been injected -> open-loop / training / pure-Python paths are unchanged.
                rx_gain = C_info['gain']
                mg = getattr(self, '_measured_gain', None)
                if mg is not None and self._gain_fresh[uav_idx_assoc, ed_idx]:
                    rx_gain = float(mg[uav_idx_assoc, ed_idx])
                snr_val = compute_snr(chosen_tp_dbm, ed_pos, uav_pos_assoc, self.config['frequency'],
                                        gain_linear=rx_gain, sigma_noise_w=self.sigma_noise_w)
...
#############################################################################################################################
                ...
                if ed_idx not in self.system_unique_eds_ever:
                    ...
                    per_uav_new_eds_this_step[uav_idx_assoc] += 1   
        ...
        # Per-served-ED normalized EE: divide each UAV's EE by the #EDs it served this
        # step (guard divide-by-zero -> 0 if it served none). This measures *efficiency
        # per served device* so the term no longer declines as coverage grows (raw
        # aggregate power grows faster than rate when more EDs are served).
        ...
        # Weighted system, equal weights lambda_u = 1/U (sum_u lambda_u = 1)
        ...
        omega1 = self.config['reward_weights']['omega1_association_indicator']      # ω1 association
        omega2 = self.config['reward_weights']['omega2_coverage_bonus']             # ω2 (B) coverage
        omega3 = self.config['reward_weights']['omega3_system_ee_step']             # ω3 system EE
        omega4 = self.config['reward_weights']['omega4_spread_incentive']           # ω4 (D) spread
        omega5 = self.config['reward_weights']['omega5_collision_penalty']          # ω5 collision
        omega6 = self.config['reward_weights']['omega6_distance_to_cs_penalty']     # ω6 CS distance
        omega7 = self.config['reward_weights']['omega7_cs_arrival_failure_penalty'] # ω7 terminal CS-fail
        spread_cap = self.config['reward_weights']['spread_cap']                    # δ_cap (D)

        # Accumulate raw EE_sys once per step
        ...
        # (D) Inter-UAV spread: distance from each UAV to its NEAREST other UAV. Rewarding
        # this (capped) breaks the parallel-corridor symmetry so the UAVs fan out laterally
        # and sweep more of the field. Capped + modest so the omega6 CS-pull still lands them.
        ...
        # --- per-agent reward (the CONVERGING formulation) ---
        for uav_idx in range(self.n_agents):
            if self.dones[uav_idx] and not uav_collided_this_step[uav_idx]:
                pass  # Agent was already done and didn't just collide

            # Coverage-aware association: base indicator omega1*a_u PLUS a bonus for each
            ...
            info['individual_rewards_components'][uav_idx]['r_association (omega1*a_u + omega2*new_eds)'] = r_association
            info['individual_rewards_components'][uav_idx]['r_coverage (omega2*new_eds)'] = r_coverage

            # Spread reward: rewards separation from the nearest other UAV, saturating at spread_cap.
            ...
            self.episode_sum_weighted_r_coverage += r_coverage  
            self.episode_sum_weighted_r_spread += r_spread      
        ...
        if episode_done_signal:
            omega_cs_fail = self.config['reward_weights']['omega7_cs_arrival_failure_penalty'] 
            for uav_idx in range(self.n_agents):
    ...
    def get_stats(self):
        ...
        # --- Packet-capture metrics from the 3-gate LoRa capture model (per episode) ---
        ...
        # Add stats for CS arrival failure penalty
...
# # Only runs when this file is executed directly (python multi_lora.py),
# # NOT on import by EPyMARL.
# if __name__ == "__main__":
#     # Define area_size globally if generate_uav_positions needs it as a fallback
#     area_size = [1000, 1000]
#     env = MultiFlyingLoRaEnv(num_uavs=4, num_eds=60, area_size=area_size, uav_altitude=150, max_episode_steps=400, max_speed=30, comm_range=300, safe_distance=3)

#     print("Observation Space:", env.observation_space)
#     print("Action Space:", env.action_space)
#     print("Number of actions per agent:", env.n_actions)

#     obs = env.reset()
#     print("Initial Observation (Agent 0 Shape):", np.shape(obs[0]))

#     for step_num in range(5):
#         # tuple of MultiDiscrete vectors [dir, speed, (sf_k, tp_k) for k in 0..M-1], one per agent.
#         # NOTE: sample() is unmasked, so it may pick non-null SF/TP on padded slots; step()
#         # ignores those via observed-only association. Real training masks via get_avail_actions().
#         action = env.action_space.sample()
#         obs, reward, done, info = env.step(action)
#         print(f"\n--- Step {env.timestep} ---")

#         # --- Per-ED factored action display (Agent 0) ---
#         action_vec_agent0 = action[0]                       # [dir, speed, (sf_k, tp_k)*M]
#         decoded = env._decode_action(action_vec_agent0)

#         print(f"Action Vector (Agent 0): {action_vec_agent0}")

#         dir_idx   = decoded['direction_idx']
#         speed_idx = decoded['speed_idx']
#         sf_raw    = decoded['sf_raw']   # per-slot; 0 = null/decline, else sf_options[idx-1]
#         tp_raw    = decoded['tp_raw']

#         # Actual direction angle relative to CS vector (mirrors 'theta_relative_to_cs' in step()).
#         beta = math.radians(env.config['movement_directions_angle'] / 2)
#         if env.config['movement_directions'] > 1:
#             actual_relative_angle_deg = math.degrees(
#                 -beta + 2 * beta * (dir_idx / (env.config['movement_directions'] - 1))
#             )
#         else:
#             actual_relative_angle_deg = 0.0  # Only one direction possible

#         actual_speed_value = env.config['speed_steps'][speed_idx] * env.max_speed

#         print("  Actual Action Values (Agent 0):")
#         print(f"    Direction (relative to CS): {actual_relative_angle_deg:.2f} degrees")
#         print(f"    Speed: {actual_speed_value:.2f} m/s")
#         print(f"    Per-ED SF/TP decisions ({env.max_ed_per_uav} slots; 0=decline):")
#         served_slots = 0
#         for k in range(env.max_ed_per_uav):
#             if sf_raw[k] == 0 or tp_raw[k] == 0:
#                 continue
#             sf = env.config['sf_options'][sf_raw[k] - 1]
#             tp = env.config['tp_options'][tp_raw[k] - 1]
#             print(f"      slot {k:2d}: SF{sf}  TP {tp} dBm")
#             served_slots += 1
#         print(f"      -> {served_slots} slot(s) requested service, "
#               f"{env.max_ed_per_uav - served_slots} declined/null")
#         # --- end display ---

#         print("Reward (All Agents):", reward)
#         print("Done (Episode):", done)
#         get_stats = env.get_stats()
#         print("Stats (Global):")
#         for k, v in get_stats.items():
#             print(f"  {k}: {v}")
#         # if step_num == 0: print("Info:", info) # Print full info for first step
#         if done:
#             print("Episode finished after", env.timestep, "timesteps.")
#             print("Stats:")
#             for k, v in env.get_stats().items():
#                 print(f"  {k}: {v}")
#             obs = env.reset() # Reset for next loop iteration if episodes are short

#         # print("*" * 40)
#         # print("  Per-ED SNR (active associations this step):")
#         # if env.ed_snr_this_step:
#         #     for ed_idx in sorted(env.ed_snr_this_step.keys()):
#         #         r = env.ed_snr_this_step[ed_idx]
#         #         flag = "PASS" if r['passed'] else "drop"
#         #         print(f"    ED {ed_idx:3d} -> UAV {r['uav']} | SF{r['sf']} | TP {r['tp']:2d} dBm | "
#         #               f"SNR {r['snr_db']:6.2f} dB (thr {r['threshold_db']:5.1f}) [{flag}]")
#         # else:
#         #     print("    (no EDs associated this step)")
