...
def _multidiscrete_log_pi_and_entropy(pi, actions, action_nvec):
    """Per-dim log-prob (summed) and entropy (summed) for MultiDiscrete.

    pi:      (B, T, A, sum(nvec))  — per-dim softmax probs concatenated
    actions: (B, T, A, n_dims)     — one index per dimension
    Returns: log_pi_taken (B, T, A), entropy (B, T, A)
    """
    log_pi_taken = th.zeros(pi.shape[:-1], device=pi.device)
    entropy = th.zeros(pi.shape[:-1], device=pi.device)
    start = 0
    for i, n in enumerate(action_nvec):
        pi_dim = pi[..., start:start + n]                          # (B, T, A, n)
        idx = actions[..., i:i + 1]                                # (B, T, A, 1)
        taken = th.gather(pi_dim, dim=-1, index=idx).squeeze(-1)   # (B, T, A)
        log_pi_taken = log_pi_taken + th.log(taken + 1e-10)
        entropy = entropy - th.sum(pi_dim * th.log(pi_dim + 1e-10), dim=-1)
        start += n
...
class PPOLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        # --- optional t_env-based linear LR decay (off by default) ---
        # enable with:  with lr_decay=True   (optionally lr_decay_min_frac=0.1)
        # lr(t) = lr0 * (1 - (1 - min_frac) * min(t_env / t_max, 1))   -> lr0 -> lr0*min_frac
        self.lr_decay = getattr(args, "lr_decay", False)
        self.start_lr = args.lr
        self.lr_decay_min_frac = getattr(args, "lr_decay_min_frac", 0.1)
        _steps = getattr(args, "lr_decay_steps", 0)
        self.lr_decay_steps = _steps if _steps and _steps > 0 else getattr(args, "t_max", 1_000_000)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # Optional LR decay: set both optimisers' lr from the env-step count.
        if self.lr_decay:
            frac = min(float(t_env) / max(1, self.lr_decay_steps), 1.0)
            cur_lr = self.start_lr * (1.0 - (1.0 - self.lr_decay_min_frac) * frac)
            for opt in (self.agent_optimiser, self.critic_optimiser):
        ...
        _action_nvec = getattr(self.args, "action_nvec", None)
        if _action_nvec is not None:
            old_log_pi_taken, _ = _multidiscrete_log_pi_and_entropy(old_pi, actions, _action_nvec)
        else:
        ...
        for k in range(self.args.epochs):
            ...
            if _action_nvec is not None:
                log_pi_taken, entropy = _multidiscrete_log_pi_and_entropy(pi, actions, _action_nvec)
            else:
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("lr", self.agent_optimiser.param_groups[0]["lr"], t_env)
