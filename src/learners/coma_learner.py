...
def _md_actor_gather(q_vals, pi, actions_flat, action_nvec):
    """Per-dim gather for COMA actor; returns q_taken, log_pi_taken (both flattened)."""
    q_taken = th.zeros(q_vals.shape[0], device=q_vals.device)
    log_pi_taken = th.zeros(pi.shape[0], device=pi.device)
    start = 0
    for i, n in enumerate(action_nvec):
        q_dim = q_vals[:, start:start + n]
        p_dim = pi[:, start:start + n]
        a_idx = actions_flat[:, i:i + 1]
        q_taken = q_taken + th.gather(q_dim, dim=1, index=a_idx).squeeze(1)
        log_pi_taken = log_pi_taken + th.log(th.gather(p_dim, dim=1, index=a_idx).squeeze(1) + 1e-10)
        start += n
    return q_taken, log_pi_taken


def _md_critic_gather(q_vals, actions, action_nvec):
    """Per-dim gather for COMA critic training; returns q_taken of shape (B, T, A)."""
    result = th.zeros(q_vals.shape[:-1], device=q_vals.device)
    start = 0
    for i, n in enumerate(action_nvec):
        q_dim = q_vals[..., start:start + n]
        a_dim = actions[..., i:i + 1]
...
class COMALearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if _action_nvec is not None:
            actions_flat = actions.reshape(-1, len(_action_nvec))
            q_taken, log_pi_taken = _md_actor_gather(q_vals, pi, actions_flat, _action_nvec)
    ...
    def _train_critic(
        ...
        _action_nvec = getattr(self.args, "action_nvec", None)
        if _action_nvec is not None:
            targets_taken = _md_critic_gather(target_q_vals, actions, _action_nvec)
        else:
        ...
        if _action_nvec is not None:
            q_taken = _md_critic_gather(q_vals, actions, _action_nvec)
        else:
