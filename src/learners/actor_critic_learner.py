...
# MultiDiscrete (factored per-ED action heads) log-prob/entropy helper -- single source
# of truth shared with the PPO learner, so MAA2C supports the factored action space.
from learners.ppo_learner import _multidiscrete_log_pi_and_entropy
...
class ActorCriticLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # MultiDiscrete: sum log-probs/entropy across the per-ED action heads so
        # log_pi_taken is PER-AGENT and matches the advantages (fixes the (n_agents vs
        # n_heads) shape crash). Falls back to single-discrete when action_nvec is None.
